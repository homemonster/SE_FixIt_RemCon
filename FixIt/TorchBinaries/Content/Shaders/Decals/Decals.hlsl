// @define USE_COLORMETAL_TEXTURE 
// @define USE_NORMALGLOSS_TEXTURE
// @define RENDER_TO_TRANSPARENT
// @define USE_EXTENSIONS_TEXTURE
...
{
    screencoord -= frame_.Screen.offset;
...
void __pixel_shader(VsOut vertex, out GbufferOutput output)
{
    ...
    float shadow = CalculateShadow(decalPosV, 1);
    float4 decalColor = float4(shadow.rrr, 1) * decalCmRGBA;
    float ambientFarFactor = GetAmbientFarFactor(-depth);
    float3 ambient = AmbientDiffuse(1, decalN, ambientFarFactor) * frame_.Light.ambientDiffuseFactor;
    decalColor.rgb += ambient * decalColor.a;
    TransparentColorOutput(decalColor, depth, vertex.position.z, 1.0f, accumTarget, coverageTarget);
#else
    if (abs(dot(decalN, gbufferN)) < 0.33)
        discard;
    float decalAlpha = ReadAlphamaskTexture(texcoord);
