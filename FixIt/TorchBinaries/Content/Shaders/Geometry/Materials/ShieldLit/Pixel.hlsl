...
float GetSpecularLight(float3 N, float3 V, float f0, float gloss)
{
    float3 L = -frame_.Light.directionalLightVec;
    float3 H = normalize(V + L);
    float ln = saturate(dot(L, N));
    float vn = saturate(dot(V, N));
    float nh = saturate(dot(N, H));
    float vh = saturate(dot(V, H));
    return SpecularPhongS(ln, nh, vn, vh, f0, gloss);
}
...
void pixel_program(PixelInterface pixel, inout MaterialOutputInterface output)
{
    ...
    float specularWhitenessFalloff = TransparentConstants.LightMultiplier.w;
    float specularIntensity = TransparentConstants.SpecularColorFactor;
    ...
    float backLight = pow(highlightFactor * backLightStrength, backLightFalloff);

    // specular light
    float specularFactor = GetSpecularLight(output.normal, viewVector, 1.0f, gloss);
	float3 specular = saturate(specularFactor * textureSample * userColor * specularWhitenessFalloff) * specularIntensity;

    // specular light's shadow
