...
#define SpecularLightT SpecularPhongT
#define DiffuseLight DiffuseLightOrenNayar
#define DiffuseLightT DiffuseLightOrenNayarT
...
float3 MaterialRadiance(float3 albedo, float3 sColor, float3 f0, float gloss, float3 N, float3 L, float3 V, float3 H, float dFactor, float sFactor = 1)
{
    ...
    float3 diffuseLight = DiffuseLight(ln, vn, lv, albedo, gloss);
...
float4 MaterialRadianceT(float4 albedo, float4 sColor, float4 f0, float gloss, float3 N, float3 L, float3 V, float3 H, float dFactor, float sFactor = 1)
{
    ...
    float4 diffuseLight = DiffuseLightT(ln, vn, lv, albedo, gloss);
	float4 specularLight = SpecularLightT(ln, nh, vn, vh, f0, gloss);
