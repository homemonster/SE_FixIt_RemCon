...
// @defineMandatory MAX_OFFSET 5,MAX_OFFSET 8
// @defineMandatory DENSITY_FUNCTION 0, DENSITY_FUNCTION 1
// @define DEPTH_DISCARD_THRESHOLD 0.2, DEPTH_DISCARD_THRESHOLD 0.1
...
void __pixel_shader(PostprocessVertex input, out float4 output : SV_Target0)
{
    ...
    if ( !IsDepthForeground(depthSample))
    {
    ...
    for ( int offsetIndex = -size; offsetIndex <= size; ++offsetIndex )
    {
        ...
        if ( !IsDepthForeground(depthSampleAtOffset) || (abs(linearDepth - linearDepthAtOffset) > DEPTH_DISCARD_THRESHOLD) )
        {
