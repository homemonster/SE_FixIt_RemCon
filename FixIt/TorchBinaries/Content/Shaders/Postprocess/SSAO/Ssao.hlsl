...
// sample depth
		...
		[unroll] for(j=0; j<4; j++) {
			...
			float2 screencoord = (frame_.Screen.offset + clamp(occlProjPos[j] * frame_.Screen.resolution, 0, frame_.Screen.resolution - 1)) / frame_.Screen.resolution;
...
/*	float margin = 1.f;
	...
	vignette_dist = min(vignette_dist, saturate((frame_.Screen.resolution - input.position.xy - frame_.Screen.offset) / margin));
