...
{ 
    ...
    float3 colorSum = 0 ; 
    float weight = 1.0 ; 
    float exposure = exp2 ( Exposure . Load ( uint3 ( 0 , 0 , 0 ) ) . y ) ; 
    [ unroll ] 
    for ( uint i = 0 ; i < 4 ; i ++ ) 
    { 
        float3 linearColor = Sample ( texel + offsets [ i ] ) . rgb * exposure ; 
        
        float totalLuminance = GetRelativeLuminance ( linearColor ) ; 
