...
{ 
    float alphaSample = AlphamaskTexture . Sample ( Sampler , input . uv * 2 ) * Color . w * 0.5f ; 
    if ( alphaSample < 0.001 ) 
    discard ; 
...
#line 84
    float shadingMultiplier = pow ( saturate ( - dot ( input . normal , frame_ . Light . directionalLightVec ) + 0.3 ) , 3 ) + 0.001 ; 
    
    output = float4 ( colorSample * shadingMultiplier , saturate ( alphaSample ) ) ; 
} 
...
#line 123
    ...
    float highestFogDensity = 0.03 ; 
    float fogDensity = lerp ( 0 , highestFogDensity , 1 - saturate ( abs ( CameraAltitude - LayerAltitude ) / ( LayerThickness / 2 ) ) ) ; 
    float fogMultiplier = FogUniform ( max ( 1 - depthSample * 500 , 0 ) , fogDensity ) ; 
    
    float4 fogColor = float4 ( frame_ . Fog . color , 1 ) ; 
