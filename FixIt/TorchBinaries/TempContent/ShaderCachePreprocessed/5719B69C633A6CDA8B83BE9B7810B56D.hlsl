...
#line 3 ""
Texture2D < float > Shadows : register ( t19 ) ; 

void __pixel_shader ( PostprocessVertex vertex , out float4 output : SV_Target0 ) 
{ 
    float shadow = Shadows [ vertex . position . xy ] . x ; 
    ...
    float3 lPos ; 
    float distance = length ( input . position ) ; 
    uint cascadeIndex = CalculateCascadeIndexFromSplit ( distance , input . position , lPos ) ; 
    GetHigherCascadeShadow ( input . position , cascadeIndex , lPos ) ; 
    
    output = float4 ( CASCADE_DEBUG_COLORS [ cascadeIndex ] * shadow , 1 ) ; 
} 
