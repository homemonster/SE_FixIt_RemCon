...
void SpawnBillboard ( float3 position , float3 InstancePosition , float2 scale , float3 surfaceNormal , float3 bentNormal , float3 surfaceTangent , uint textureIndex , inout TriangleStream < RenderingPixelInput > triangle_stream ) 
{ 
    RenderingPixelInput vertex ; 
    scale . x *= 0.5 ; 
    
    float3 windOffset = CalculateWindOffset ( InstancePosition ) ; 
    float3 adjustedNormal = normalize ( bentNormal + windOffset ) ; 
...
{ 
    ...
    { 
        ...
        float dist = viewVectorLength / frame_ . Foliage . clipping_scaling . z * object_ . foliage_multiplier ; 
        int part = int ( dist * PARTS_COUNT ) ; 
        bool skip = GetSkip ( seed , part ) ; 
        bool skipNext = GetSkip ( seed , part + 1 ) ; 
        if ( skip != skipNext ) 
        { 
            float distFraction = 1 - ( dist * PARTS_COUNT - part ) ; 
            scale *= distFraction ; 
        } 
        ...
        const float angle_min = 0.5f * 0.785398163397448309616 ; 
        const float angle_max = 0.75f * 1.57079632679489661923 ; 
        ...
        float cameraAngle = dot ( viewVectorDirection , surfaceNormal ) ; 
        if ( cameraAngle > 0.5 ) 
        { 
            float rotateBy = ( cameraAngle - 0.5 ) * 1.5 ; 
            
            float dotView = dot ( onb [ 1 ] , viewVectorDirection ) ; 
            if ( dotView > - 0.05f && dotView < 0.05f ) 
            rotateBy = rotateBy * dotView * 20 ; 
            else if ( dotView < - 0.05f ) 
            rotateBy = - rotateBy ; 
            onb = mul ( rotate_x ( rotateBy ) , onb ) ; 
        } 
        if ( ! skip ) 
        SpawnBillboard ( position . xyz , input [ 0 ] . InstancePosition . xyz , scale , surfaceNormal , onb [ 2 ] , onb [ 0 ] , textureIndex , triangle_stream ) ; 
