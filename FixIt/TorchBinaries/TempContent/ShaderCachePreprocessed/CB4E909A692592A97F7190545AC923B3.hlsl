...
{ 
    float4 position : POSITION0 ; float4 blend_weights : BLENDWEIGHT0 ; uint4 blend_indices : BLENDINDICES0 ; uint2 normal : NORMAL0 ; float4 tangent4 : TANGENT0 ; float2 texcoord0 : TEXCOORD0 ; 
} ; 
...
#line 126
    ...
    __position_object = unpack_position_and_scale ( input . position ) ; __blend_weights = input . blend_weights ; __blend_indices = input . blend_indices ; __normal = unpack_normal ( input . normal ) ; __tangent = unpack_tangent_sign ( input . tangent4 ) ; __texcoord0 = input . texcoord0 ; 
