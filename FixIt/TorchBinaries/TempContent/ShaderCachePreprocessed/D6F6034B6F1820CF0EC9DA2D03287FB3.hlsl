...
#line 186
    ...
    float3 preIntegralRayleigh = PhaseRayleigh ( cosTheta ) * BetaRayleighScattering / ( 4.0f * 3.14159265358979323846 ) ; 
    float3 preIntegralMie = PhaseMie ( cosTheta ) * BetaMieScattering / ( 4.0f * 3.14159265358979323846 ) ; 
...
#line 292
    ...
    float3 color = lerp ( integral , ToGrayScale ( integral ) , DesaturationFactor ) ; 
    float atmosphereBoost = t * 2.5f * Intensity ; 
    
    return float4 ( color * atmosphereBoost , 1.0 ) ; 
} 
...
#line 44
    output = ComputeAtmosphere ( input . V , input . position , frame_ . Light . directionalLightVec , input . depth , input . native_depth , 2 ) ; 
} 
